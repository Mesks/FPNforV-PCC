...
def data_clean(x, y):
    ...
    counter0 = counter1 = counter2 = counter3 = counter4 = counter5 = counter6 = counter7 = min([counter[0],np.sum(counter[1:35]),np.sum(counter[35:48]),np.sum(counter[48:58]),np.sum(counter[58:72]),np.sum(counter[72:90]),np.sum(counter[90:120]),np.sum(counter[120:481])])
    all_count        = int(8 * counter0)
    real_counter     = 0
    ...
    for i in range(y.shape[0]):
        ...
        if (now_sum == 0) & (counter0>0):
            ...
            counter0             -= 1
            real_counter         += 1
        elif (now_sum>=1) & (now_sum<35) & (counter1>0):
            after_y[real_counter] = y[i]
            after_x[real_counter] = x[i]
            counter1             -= 1
            real_counter         += 1
        elif (now_sum>=35) & (now_sum<48) & (counter2>0):
            after_y[real_counter] = y[i]
            after_x[real_counter] = x[i]
            counter2             -= 1
            real_counter         += 1
        elif (now_sum>=48) & (now_sum<58) & (counter3>0):
            after_y[real_counter] = y[i]
            after_x[real_counter] = x[i]
            counter3             -= 1
            real_counter         += 1
        elif (now_sum>=58) & (now_sum<72) & (counter4>0):
            after_y[real_counter] = y[i]
            after_x[real_counter] = x[i]
            counter4             -= 1
            real_counter         += 1
        elif (now_sum>=72) & (now_sum<90) & (counter5>0):
            after_y[real_counter] = y[i]
            after_x[real_counter] = x[i]
            counter5             -= 1
            real_counter         += 1
        elif (now_sum>=90) & (now_sum<120) & (counter6>0):
            after_y[real_counter] = y[i]
            after_x[real_counter] = x[i]
            counter6             -= 1
            real_counter         += 1
        elif (now_sum>=120) & (now_sum<481) & (counter7>0):
            after_y[real_counter] = y[i]
            after_x[real_counter] = x[i]
            counter7             -= 1
            real_counter         += 1
...
#data process
print("\nAttrI training begin: ")
save_file       = "/mnt/disk0/qsc/nnTrainingResult/AI/logs_FPN_AttrI"
writer          = SummaryWriter(save_file)
# toriPath = '/mnt/disk0/qsc/nnDataset/basketball_player_simpleTest/AttrI_TOri.csv'
# occuPath = '/mnt/disk0/qsc/nnDataset/basketball_player_simpleTest/AttrI_Occu.csv'
# y_QPPath = '/mnt/disk0/qsc/nnDataset/basketball_player_simpleTest/AttrI_y_QP.csv'
toriPath = '/mnt/disk0/qsc/nnDataset/basketball_player32/AttrI_TOri.csv'
occuPath = '/mnt/disk0/qsc/nnDataset/basketball_player32/AttrI_Occu.csv'
y_QPPath = '/mnt/disk0/qsc/nnDataset/basketball_player32/AttrI_y_QP.csv'
...
for epoch in range(max_epoch):
    ...
    model_save_path = save_file+"/model_AttrI_"+str(epoch)+".pt"
